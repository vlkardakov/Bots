        ...
        for i in range(10):
            if is_lobby():
                ...
                if text != (""or" ") and author != "":
                    #send_chat(f"""Уважаемый {author}, Ваш запрос принят.{Enter}""")
                    journal.append(f"{time.time()}#{author}#{text}")
                    a = gpt(author, text, load_memory(author))
                    journal.append(f"{time.time()}#BOT#{a}#{author}")
                    #time.sleep(3.5)
                    ans, memory = a
                    print(memory)
                    save_memory(author, memory)
                    print(ans)
                    send_chat(str(ans)+"{Enter}")
...
if __name__ == "__main__":
    send_chat("Вас приветствует BOT 01. Чтобы задать вопрос, начните предложение со слова бот.  {Enter}")
    while True:
        try:
            main()
        except Exception as e:
